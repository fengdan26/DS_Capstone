...
class Generator(nn.Module):
    def __init__(self):
        ...
        def block(in_channels, out_channels, kernel_size=8, stride=2, padding=1):
        ...
        self.model = nn.Sequential(
            block(opt.channels + opt.noise_dim, 64, 4, 2, 1),
            block(64, 128, 4, 2, 1),
            block(128, 256, 4, 2, 1),
            nn.ConvTranspose2d(256, opt.channels, kernel_size=4, stride=2, padding=1),  # 保持通道数与目标一致
            nn.Tanh(),
        )
        self.output_resize = nn.AdaptiveAvgPool2d((opt.img_size, opt.img_size))

    def forward(self, conditions, noise):
        input = torch.cat([conditions, noise], dim=1)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            block(opt.channels, 64, 4, 2, 1),
            block(64, 128, 4, 2, 1),
            block(128, 256, 4, 2, 1),
            nn.Flatten(),
            nn.Dropout(0.2),  # 添加 Dropout
            nn.Linear(256 * (opt.img_size // 8) ** 2, 1),
            nn.Sigmoid(),
        )
